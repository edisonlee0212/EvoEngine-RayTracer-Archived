...
void RayTracer::CreateHitGroupPrograms() {
	{
		...
		OPTIX_CHECK(optixProgramGroupCreate(
			m_optixDeviceContext, &pgDesc, 1, &pgOptions, log, &sizeofLog,
			&m_cameraRenderingPipeline.m_hitGroupProgramGroups[RayType::Radiance][PrimitiveType::CubicBezier]));
		...
		OPTIX_CHECK(optixProgramGroupCreate(
			m_optixDeviceContext, &pgDesc, 1, &pgOptions, log, &sizeofLog,
			&m_cameraRenderingPipeline.m_hitGroupProgramGroups[RayType::SpacialSampling][PrimitiveType::CubicBezier]));
	...
	}
	{
		...
		OPTIX_CHECK(optixProgramGroupCreate(
			m_optixDeviceContext, &pgDesc, 1, &pgOptions, log, &sizeofLog,
			&m_illuminationEstimationPipeline.m_hitGroupProgramGroups[RayType::Radiance][PrimitiveType::CubicBezier]));
		if (sizeofLog > 1)
			std::cout << log << std::endl;
		// -------------------------------------------------------
		// BSSRDF Sampler ray
		...
		OPTIX_CHECK(optixProgramGroupCreate(
			m_optixDeviceContext, &pgDesc, 1, &pgOptions, log, &sizeofLog,
			&m_illuminationEstimationPipeline.m_hitGroupProgramGroups[RayType::SpacialSampling][PrimitiveType::CubicBezier]));
		if (sizeofLog > 1)
			std::cout << log << std::endl;
	}
	{
		...
		OPTIX_CHECK(optixProgramGroupCreate(
			m_optixDeviceContext, &pgDesc, 1, &pgOptions, log, &sizeofLog,
			&m_pointCloudScanningPipeline.m_hitGroupProgramGroups[RayType::Radiance][PrimitiveType::CubicBezier]));
		if (sizeofLog > 1)
			std::cout << log << std::endl;
		// -------------------------------------------------------
		// BSSRDF Sampler ray
		...
		OPTIX_CHECK(optixProgramGroupCreate(
			m_optixDeviceContext, &pgDesc, 1, &pgOptions, log, &sizeofLog,
			&m_pointCloudScanningPipeline.m_hitGroupProgramGroups[RayType::SpacialSampling][PrimitiveType::CubicBezier]));
		if (sizeofLog > 1)
			std::cout << log << std::endl;
	}
}
...
__global__ void
CopyVerticesInstancedKernel(int matricesSize, int verticesSize, InstanceMatrix* matrices,
	EvoEngine::Vertex* vertices,
	glm::vec3* targetPositions, EvoEngine::Vertex* targetVertices) {
	const int idx = threadIdx.x + blockIdx.x * blockDim.x;
	if (idx < verticesSize * matricesSize) {
		const glm::vec3 position = matrices[idx / verticesSize].m_instanceMatrix *
...
__global__ void
CopyStrandPointsKernel(int size, EvoEngine::StrandPoint* strandPoints, float* targetThicknesses) {
	const int idx = threadIdx.x + blockIdx.x * blockDim.x;
	if (idx < size) {
...
__global__ void
CopyVerticesKernel(int size, EvoEngine::Vertex* vertices,
...
__global__ void CopySkinnedVerticesKernel(int size,
	EvoEngine::SkinnedVertex* vertices,
	glm::mat4* boneMatrices,
	glm::vec3* targetPositions,
	EvoEngine::Vertex* targetVertices) {
	const int idx = threadIdx.x + blockIdx.x * blockDim.x;
	if (idx < size) {
...
#pragma region Geometry Inputs
	...
	case RendererType::Curve: {
		...
		CopyStrandPointsKernel << <gridSize, blockSize >> > (
			size,
			static_cast<EvoEngine::StrandPoint*>(m_vertexDataBuffer.m_dPtr),
		...
		case PrimitiveType::CubicBezier:
			buildInput.curveArray.curveType = OPTIX_PRIMITIVE_TYPE_ROUND_CUBIC_BEZIER;
			break;
		}
		...
		buildInput.curveArray.vertexStrideInBytes = sizeof(EvoEngine::StrandPoint);
		...
		buildInput.curveArray.indexStrideInBytes = sizeof(glm::uvec4);
	...
	case RendererType::Default: {
		...
		buildInput.triangleArray.vertexStrideInBytes = sizeof(EvoEngine::Vertex);
	...
	case RendererType::Skinned: {
		...
		m_vertexDataBuffer.Resize(
			m_skinnedVertices->size() * sizeof(EvoEngine::Vertex));
		...
		CopySkinnedVerticesKernel << <gridSize, blockSize >> > (
			size,
			static_cast<EvoEngine::SkinnedVertex*>(skinnedVerticesBuffer.m_dPtr),
			static_cast<glm::mat4*>(boneMatricesBuffer.m_dPtr),
			static_cast<glm::vec3*>(devicePositionBuffer.m_dPtr),
			static_cast<EvoEngine::Vertex*>(m_vertexDataBuffer.m_dPtr));
		CUDA_SYNC_CHECK();
		m_triangleBuffer.Upload(*m_triangles);
		buildInput = {};
		buildInput.type = OPTIX_BUILD_INPUT_TYPE_TRIANGLES;
	...
	case RendererType::Instanced: {
		...
		m_vertexDataBuffer.Resize(m_instanceMatrices->size() *
			m_vertices->size() * sizeof(EvoEngine::Vertex));
		...
		CopyVerticesInstancedKernel << <gridSize, blockSize >> > (matricesSize,
			verticesSize,
			static_cast<InstanceMatrix*>(instanceMatricesBuffer.m_dPtr),
			static_cast<EvoEngine::Vertex*>(verticesBuffer.m_dPtr),
			static_cast<glm::vec3*>(devicePositionBuffer.m_dPtr),
			static_cast<EvoEngine::Vertex*>(m_vertexDataBuffer.m_dPtr));
		CUDA_SYNC_CHECK();
...
void RayTracedGeometry::UploadForSBT() {
	...
	if (m_geometryType != PrimitiveType::Triangle) {
		...
		TriangularMesh mesh;
		mesh.m_vertices = reinterpret_cast<EvoEngine::Vertex*>(m_vertexDataBuffer.DevicePointer());
...
void RayTracer::CreateModule(RayTracerPipeline& targetPipeline, char ptxCode[],
	...
	targetPipeline.m_pipelineCompileOptions.usesPrimitiveTypeFlags =
		...
		OPTIX_PRIMITIVE_TYPE_FLAGS_ROUND_QUADRATIC_BSPLINE | OPTIX_PRIMITIVE_TYPE_FLAGS_ROUND_CUBIC_BEZIER;
...
#pragma region Material Settings
		...
		if (m_albedoTexture) material.m_albedoTexture = m_albedoTexture->m_textureObject;
		else material.m_albedoTexture = 0;
		if (m_normalTexture) material.m_normalTexture = m_normalTexture->m_textureObject;
		else material.m_normalTexture = 0;
		if (m_roughnessTexture) material.m_roughnessTexture = m_roughnessTexture->m_textureObject;
		else material.m_roughnessTexture = 0;
		if (m_metallicTexture) material.m_metallicTexture = m_metallicTexture->m_textureObject;
		else material.m_metallicTexture = 0;
